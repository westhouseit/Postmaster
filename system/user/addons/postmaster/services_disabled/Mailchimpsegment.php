...
class MailchimpSegment_postmaster_service extends Mailchimp_postmaster_service {
	...
	public function __construct()
	{
		...
		foreach(ee()->postmaster_model->get_tasks()->result() as $task)
		{
	...
	{
		$this->member = ee()->channel_data->get_member($parcel->entry->author_id)->row();

		$this->parse_vars = array_merge(
			(array) ee()->channel_data->utility->add_prefix('entry', $parcel->entry),
			(array) ee()->channel_data->utility->add_prefix('member', $this->member)
		);
	...
	public function get_campaign_params($list_id, $parsed_object, $parcel)
	{
		...
		{
			$task_settings = json_decode(ee()->postmaster_model->get_task($settings->task_id)->row('settings'));
