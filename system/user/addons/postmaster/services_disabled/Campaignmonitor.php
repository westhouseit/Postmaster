...
class CampaignMonitor_postmaster_service extends Base_service {
	...
	public function subscribers($data)
	{
		...
		$url    = $this->api_url('lists', $data['id'], 'active', array(
			'date'           => !empty($date) ? $date : date('Y-m-01', ee()->localize->now),
			'page'           => $data['start'],
			'pagesize'       => $data['limit'],
		...
		foreach($subscribers->Results as $index => $subscriber)
		{
			...
			$row[$data['prefix'].':data']  =  ee()->channel_data->utility->add_prefix($data['prefix'], array($result));
	...
	{	
		$post = array(
			...
			'Name'         => !isset($data['name']) ? (ee()->input->post('name', TRUE) ? ee()->input->post('name', TRUE) : $data['email']) : $data['name'],
		);
	...
	public function send($parsed_object, $parcel)
	{
		...
		$html_url = ee()->postmaster_lib->current_url('ACT', ee()->channel_data->get_action_id('Postmaster_mcp', 'template')).'&entry_id='.$parcel->entry->entry_id.'&parcel_id='.$parcel->id;
