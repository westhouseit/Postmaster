...
class MandrillMatrixMailingList_postmaster_service extends Mandrill_postmaster_service {
	...
	public function send($parsed_object, $parcel)
	{
		...
		$field       = ee()->channel_data->get_field_by_name($settings->matrix_field);
		$cols        = ee()->channel_data->get('matrix_cols', array(
			'where' => array(
				'field_id' => $field->row('field_id')
			)
		));
		$cols		 = ee()->channel_data->utility->reindex('col_name', $cols->result());
		...
		$matrix_data = ee()->channel_data->get('matrix_data', array(
			'select' => $select, 
			'where'  => array(
		...
		{
			//$prefixed_row = ee()->channel_data->utility->add_prefix($this->var_prefix, $row);
