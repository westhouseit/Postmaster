...
abstract class Base_notification extends Postmaster_base_api {
	...
	{			
		...
		if(ee()->postmaster_lib->validate_conditionals($parsed_array['extra_conditionals']))
		{
			$response = ee()->postmaster_lib->send($parsed_array, $notification);
		}
		else
		{
			$response = ee()->postmaster_lib->failed_response($$parsed_array);
		}
