		...
		*/
		...
		public function theme_path($third_party = TRUE, $append = NULL)
		{
			if($third_party)
			{
				...
				{
					return rtrim(ee()->config->item('theme_folder_path'), '/').'/third_party/' . $append;;
				}
			}
			else
			{
		...
		public function theme_url($third_party = TRUE, $append = NULL)
		{
			if($third_party)
			{
				...
				{
					return rtrim(ee()->config->item('theme_folder_url'), '/').'/third_party/' . $append;
				}
			}
			else
			{
		...
		public function javascript($file, $callback = FALSE, $debug = FALSE)
		{
			...
			if(!in_array($file, $this->loaded_files))
			{
				if($this->requirejs())
				{
					...
					ee()->requirejs->add($file, $callback);
				}
				else
				{
					...
					if(!$this->is_valid_url($file) && method_exists(ee()->cp, 'add_to_foot'))
					{
		...
		{
			return ($this->requirejs && isset(ee()->requirejs)) ? TRUE : FALSE;
		}
		...
		public function css($file)
		{
			...
			if(!in_array($file, $this->loaded_files))
			{
				...
				ee()->cp->add_to_head('<link type="text/css" href="'.$file.'?v='.time().'" rel="stylesheet" media="screen" />');
			}
		}
