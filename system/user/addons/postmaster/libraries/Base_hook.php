...
abstract class Base_hook extends Postmaster_base_api {
	...
	{
		return ee()->postmaster_hook->end_script($response);
	}
	...
	{
		return ee()->postmaster_model->get_installed_hooks($hook);
	}
	...
	{	
		...
		if(ee()->postmaster_lib->validate_conditionals($parsed_hook['extra_conditionals']))
		{
			$obj['response'] = ee()->postmaster_lib->send($parsed_hook, $hook);
		}
		else
		{
			$obj['response'] = ee()->postmaster_lib->failed_response($parsed_hook);
		}
