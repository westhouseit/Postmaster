...
class Postmaster_lib {
	...
	public function __construct()
	{
		...
		ee()->load->config('postmaster_config');
		ee()->load->model('postmaster_model');
		ee()->load->driver('channel_data');
		ee()->load->helper('postmaster_helper');
		ee()->lang->loadfile('postmaster');

		$this->model = ee()->postmaster_model;
	}
	...
	public function get_themes()
	{
		...
		$directory = ee()->theme_loader->theme_path().'postmaster/css/themes/';
		...
		foreach(directory_map($directory) as $theme)
		{
			...
			ee()->theme_loader->css('themes/'.$name);
	...
	public function parse($parcel, $member_id = FALSE, $parse_vars = array(), $prefix = 'parcel', $delimeter = ':')
	{
		...
		$channels       = ee()->postmaster_model->get_channels();
		$channel_fields = ee()->postmaster_model->get_channel_fields($channel_id);

		if(isset($parcel->entry))
		{
			if(isset($parcel->entry->entry_id))
			{
				$entry = ee()->channel_data->get_channel_entry($parcel_copy->entry->entry_id, '*')->row_array();
				$entry_vars = ee()->channel_data->utility->add_prefix($prefix, $entry, $delimeter);
			}
			else
			{
		...
		$parse_vars[$prefix.$delimeter.'safecracker'] 		  = isset(ee()->safecracker) ? TRUE : FALSE;
		$parse_vars[$prefix.$delimeter.'channel_form'] 		  = isset(ee()->channel_form) ? TRUE : FALSE;
		$parse_vars[$prefix.$delimeter.'logged_in_group_id']  = ee()->session->userdata('group_id');
		$parse_vars[$prefix.$delimeter.'logged_in_member_id'] = ee()->session->userdata('member_id');
		$parse_vars[$prefix.$delimeter.'current_time'] 		  = ee()->localize->now;
		...
		$parse_vars = array_merge($parse_vars, ee()->postmaster_model->get_member($member_id, 'member'));
		...
		//print_r($parse_vars);
		//die();
		$return = $this->convert_array(ee()->channel_data->tmpl->parse_array($parcel_copy, $parse_vars, $entry_vars, $channels, $channel_fields, $prefix.$delimeter));
		//$return = $this->convert_array(ee()->TMPL->parse_variables($parcel_copy, array_merge($parse_vars, $entry_vars, $channels, $channel_fields)));
	...
	public function route_task($task_id, $hook, $args)
	{
		...
		$routes = ee()->postmaster_routes_model->get_routes_by_task($hook, $task_id);
	...
	public function route_hook($hook_id, $hook, $args)
	{
		...
		$routes = ee()->postmaster_routes_model->get_routes_by_hook($hook, $hook_id);
	...
	private function _route($routes, $args, $type)
	{
		...
		foreach($routes->result_array() as $route)
		{
			...
			// {
				...
				{
					return ee()->postmaster_hook->trigger($route['hook'], $args);
				}
				else
				{
					return ee()->postmaster_task->trigger($route, $args);
				}
			// }
		}
	...
	{
		//echo 'postmater_lib send line 370';
		//die();
		$parsed_object = $this->convert_array($parsed_object);
		...
		if($this->validate_emails($parsed_object->to_email))
		{
			if($ignore_date || $date <= time())
			{
				...
				{
					$date = strtotime($parsed_object->send_every, ee()->localize->now);
	...
	public function trigger_hook($hook, $args)
	{
		...
		$routes = ee()->postmaster_routes_model->get_routes_by_hook($hook);
	...
	public function trigger_task_hook($hook, $args)
	{
		...
		$routes = ee()->postmaster_routes_model->get_routes_by_task($hook);
						...
						if($this->validate_trigger($parcel->trigger))
						{
							if($this->validate_categories($entry_data['category'], $parcel->categories, $parcel->match_explicitly == '1' ? true : false))
							{
								if($this->validate_member($meta['author_id'], $parcel->member_groups))
								{
									...
									{
										$entry  = ee()->channel_data->get_channel_entry($entry_id)->row();
										$parcel = $this->append($parcel, 'entry', $entry);

										$this->append($parcel, 'safecracker', (isset(ee()->safecracker) ? TRUE : FALSE));
	...
	public function should_send_email($send_once, $entry_id, $parcel_id)
	{
		...
		return ee()->postmaster_model->has_sent($entry_id, $parcel_id) ? FALSE : TRUE;
	}
	...
	public function validate_email($emails = '')
	{
		...
		{
			ee()->db->or_where('email', $email);
		}

		ee()->db->from('postmaster_blacklist');
	...
	{
		$valid  = FALSE;
		$member = ee()->channel_data->get_member($subject)->row();
	...
	public function validate_trigger($triggers)
	{
		...
		$entry_trigger = ee()->session->cache('postmaster', 'entry_trigger');
	...
	{
		$hook = ee()->extensions->in_progress;

		if(empty($hook))
		{
			$hook = ee()->session->cache('postmaster', 'in_progress');
		}
	...
	public function cp_url($method = 'index', $useAmp = FALSE)
	{
		...
		{
			return ee('CP/URL')->make('addons/settings/postmaster/'.$method);
		}
		elseif(version_compare(APP_VER, '2.8.0', '>='))
		{
			//ee('CP/URL', 'addons_modules/show_module_cp')
