...
class Doctag {
	...
	public function __construct($params = array())
	{
		...
		if(ee()->input->get('doctag_action'))
		{
			$file_name = ee()->input->get('file_name');
			$file_path = ee()->input->get('file_path');

			$snippet_name  = ee()->input->get('snippet_name');
			$snippet_value = $this->markdown($file_name, $file_path);

			$doctag_action = ee()->input->get('doctag_action');
	...
	public function markdown($str, $path = FALSE, $markdown = TRUE, $debug = FALSE)
	{
		...
		if(file_exists($str))
		{
			...
			$return = preg_replace('/'.LD.'THEME_URL'.RD.'/', ee()->config->item('theme_folder_url'), $return);
	...
	public function render_page(Doctag_Page $page)
	{
		...
		return ee()->load->view('doctag', array(
			'page' => $page,
			'tag'  => $tag,
	...
	{
		ee()->db->where('snippet_name', $name);
		$response = ee()->db->get('snippets');
	...
	public function table($directory = FALSE)
	{
		...
		ee()->table->set_heading('Tag Name', 'test');

		foreach($map as $method_name => $method)
		{
			ee()->table->add_row($method_name, 'test');
		}
...
class Doctag_Method extends Doctag {
	...
	public function __construct($name, $page)
	{
		...
		$this->url			 = $this->cp_url('doctag') . '&id='.ee()->input->get(
	'id') . '&tag='.$name;
		$this->selected		 = ee()->input->get('tag') == $name ? TRUE : FALSE;
	}
	...
	public function get_snippets($path = FALSE)
	{
		...
		ee()->table->set_heading('Snippet', 'Contents', 'Action');
		...
		if(is_dir($path))
		{
			foreach(directory_map($path) as $index => $file_name)
			{
				...
				ee()->table->add_row('<a href="#'.$snippet_name.'" class="reveal">'.$snippet_name.'</a>', character_limiter(strip_tags($snippets[$file_name]), 70), '<a href="'.$link.'">'.$link_text.'</a>');
			}
		}

		$this->snippet_table = count(ee()->table->rows) > 0 ? ee()->table->generate() : NULL;
