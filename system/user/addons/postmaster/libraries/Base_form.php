...
if(!class_exists('Base_form'))
{
	class Base_form {
		...
		public function open($hidden_fields = array(), $fields = FALSE, $entry = FALSE)
		{
			...
			if($this->return_var)
			{
				$this->return   = ee()->input->get_post('return');
			}
			...
			$this->groups			= ee()->channel_data->get_member_groups()->result_array();
			...
			{
				foreach(ee()->TMPL->tagparams as $param => $rule)
				{
			...
			$hidden_fields  = array_merge($this->hidden_fields, array(
				'site_url' => $this->param('site_url') ? $this->param('site_url') : ee()->config->item('site_url'),
			...
			{
				ee()->output->show_user_error('general', array_merge($this->field_errors, $this->errors));
			}
			...
			if($this->encode_fields)
			{
				if(is_array($fields))
				{
					foreach($fields as $index => $value)
					{
						...
						{
							$fields[$index] = ee()->encrypt->encode($value, $this->key);
						}
					}
				}
				else
				{
					$fields = ee()->encrypt->encode($fields, $this->key);
				}
			}
			...
			if($this->encode_fields)
			{
				if(is_array($fields))
				{
					foreach($fields as $index => $value)
					{
						...
						{
							$fields[$index] = ee()->encrypt->decode($value, $this->key);
						}
					}
				}
				else
				{
					$fields = ee()->encrypt->decode($fields, $this->key);
				}
			}
		...
		{
			ee()->form_validation->set_message($rule, $message);
		}
		...
		public function validate($required_fields = array(), $additional_rules = array())
		{
			if(isset($_POST[$this->validation_field]))
			{
				...
				foreach($required_fields as $field)
				{
					if(!empty($field))
					{
						ee()->form_validation->set_rules($field, ucwords(str_replace(array('-', '_'), ' ', $field)), 'callback_required_field_check');
					}
				}
				...
				foreach($rules as $field => $rule)
				{
					...
					if(is_array(ee()->input->post($field)))
					{
						$field = $field.'[]';
					}

					ee()->form_validation->set_rules($field, $label, $rule);
				}
		...
		public function redirect($group_id = FALSE)
		{
			...
			{
				$url = $this->decode(ee()->input->post('return', TRUE));
			}

			if(isset($_POST['secure_return']))
			{
				$this->secure_return = (int) $this->decode(ee()->input->post('secure_return', TRUE)) == 1 ? TRUE : FALSE;
			}

			if($group_id)
			{
				$group_redirect = $this->decode(ee()->input->post('group_'.$group_id.'_return'));
		...
		{
			return ee()->channel_data->utility->add_prefix($prefix, $data, $delimeter);
		}
	}
}
