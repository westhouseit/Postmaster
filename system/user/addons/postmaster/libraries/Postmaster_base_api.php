...
abstract class Postmaster_base_api extends Postmaster_core {
	...
	public function __construct($params = array())
	{
		...
		$this->channel_data = ee()->channel_data;
	}
	...
	{
		return ee()->postmaster_lib->failed_response($params);
	}
	...
	public function parse($array, $vars = array(), $member_data = FALSE, $entry_data = array())
	{
		...
		$vars = array_merge(array(
			...
			'logged_in_group_id'  => ee()->session->userdata('group_id'),
		), $vars);

		$vars = ee()->channel_data->utility->add_prefix($this->var_prefix, $vars);

		if(!$member_data)
		{
			$member_data = ee()->postmaster_model->get_member(FALSE);
		}

		$member_data = ee()->channel_data->utility->add_prefix('member', $member_data);

		$vars = array_merge($member_data, $vars);

		return ee()->channel_data->tmpl->parse_array($array, $vars, $entry_data, FALSE, array(), $this->var_prefix.':');
	}
