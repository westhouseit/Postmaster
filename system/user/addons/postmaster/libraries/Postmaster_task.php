...
class Postmaster_task extends Postmaster_base_lib {
	...
	{
		$route = (object) $route;
		//$routes = ee()->postmaster_routes_model->get_routes_by_task($index);
		
		$return = array();

		//foreach($routes->result() as $index => $route)
		//{
			$task 	  = ee()->postmaster_model->get_task($route->obj_id)->row_array();
			...
			if(ee()->postmaster_lib->validate_enabled($task['enabled'], 'task'))
			{
