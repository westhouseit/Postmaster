...
if(!class_exists('Channel_data_lib'))
{
	class Channel_data_lib {
		...
		private $conditionals = array('\!\=', '\<\=', '\>\=', '\<', '\>', '\=', 'like', 'LIKE', 'IS');
		...
		public function build_operator($field, $value, $protect_identifiers = TRUE, $debug = FALSE)
		{
			...
			{
				$field = ee()->db->protect_identifiers($field);
			}
			...
			return $concat . $this->remove_conditionals($field) . $conditional . ee()->db->escape($value);
		}
		...
		*/
		...
		public function get_category_entries($select = array(), $where = array(), $order_by = 'categories.cat_id', $sort = 'DESC', $limit = FALSE, $offset = 0)
		{
			...
			ee()->db->join('category_field_data', 'categories.cat_id = category_field_data.cat_id');
		...
		public function get_channel_entries($channel_id, $select = array(), $where = array(), $order_by = 'channel_titles.channel_id', $sort = 'DESC', $limit = FALSE, $offset = 0, $debug = FALSE)
		{
			...
			if($channel_id !== FALSE)
			{
				...
				ee()->db->where('channel_data.channel_id', $channel_id);
			}
			else
			{
			...
			ee()->db->join('channel_data', 'channel_titles.entry_id = channel_data.entry_id');
			...
			return ee()->db->get('channel_titles');
		}
			...
			foreach($this->reserved_terms as $term)
			{
				if(isset($params[$term]) && $params[$term] !== FALSE)
				{
					...
					if($term == 'select')
					{
						...
						{
							ee()->db->select($select, FALSE, TRUE);
						}
					}
					else if($term == 'where')
					{
						...
						if(!empty($sql)) ee()->db->where($sql, FALSE, FALSE);
					}
					...
					else if(preg_match('/^(\w*|)( |)join/', $term, $matches))
					{
						if(is_array($param))
						{
							...
							foreach($param as $index => $row)
							{
								...
								{
									ee()->db->join($table, $on, !empty($matches[1]) ? $matches[1] : false);
								}
							}
						}
					}
						...
						if(is_array($param))
						{
							...
							foreach($param as $field => $value)
							{
								...
								foreach($value as $where_val)
								{
									...
									$having_sql[] =  $concat . $this->remove_conditionals(ee()->db->protect_identifiers($where_field)) . $this->assign_conditional($where_field)  .  ee()->db->escape($where_val);
							...
							if(!empty($sql)) ee()->db->having($sql, FALSE, FALSE);
						}
					}
