...
class Channel_data_utility extends Channel_data_lib {
	...
	public function prepare_entry($channel_id, $data, $prefix = '')
	{
		...
		$fields = ee()->channel_data->get_channel_fields($channel_id);
		...
		{
			$post_value = ee()->input->post($field->field_name);
	...
	public function submit_entry($channel_id, $data)
	{
		...
		ee()->legacy_api_channel_entries->submit_new_entry($channel_id, $data);

		if(count(ee()->legacy_api_channel_entries->errors) > 0)
		{
			return ee()->legacy_api_channel_entries->errors;
		}

		return ee()->legacy_api_channel_entries->entry_id;
	}
	...
	public function update_entry($channel_id, $entry_id, $data)
	{
		...
		ee()->legacy_api_channel_entries->update_entry($entry_id, $data);
