...
class Channel_data_tmpl extends Channel_data_lib {
	...
	public function init($tagdata = FALSE)
	{
		...
		if(isset(ee()->api_channel_fields->settings))
		{
			$orig_settings = ee()->api_channel_fields->settings;
		}
		...
		if($orig_settings)
		{
			ee()->api_channel_fields->settings = $orig_settings;
		}
	...
	public function parse_array($parse_array = array(), $parse_vars = array(), $entry_data  = array(), $channels = FALSE, $channel_fields = array(), $prefix = '')
	{
		if(!$channel_fields)
		{
			...
			$channel_fields = ee()->channel_data->utility->reindex($channel_fields, 'field_name');
		}

		if(!$channels)
		{
			$channels = $this->get_channels()->result_array();
			$channels = ee()->channel_data->utility->reindex($channels, 'channel_id');
		}
	...
	public function parse($parse_vars = array(), $entry_data = array(), $channels = FALSE, $channel_fields = FALSE, $tagdata = FALSE, $prefix = '', $index = FALSE)
	{
		...
		ee()->TMPL->template = ee()->functions->prep_conditionals(ee()->TMPL->template, array_merge($parse_vars, (array) $entry_data));

		ee()->TMPL->template = ee()->TMPL->parse_variables(ee()->TMPL->template, $parse_vars);

		ee()->TMPL->template = $this->parse_fieldtypes($entry_data, $channels, $channel_fields, ee()->TMPL->template, $prefix, $index);

		ee()->TMPL->parse(ee()->TMPL->template);

		ee()->TMPL->template = ee()->TMPL->parse_globals(ee()->TMPL->template);

		$return = ee()->TMPL->template;
	...
	public function parse_path_variables($vars = array(), $entry_data = array(), $tagdata = FALSE, $prefix = '')
	{
		foreach($vars as $key => $value)
		{
			...
			{
				$path = (ee()->functions->extract_path($key) != '' AND ee()->functions->extract_path($key) != 'SITE_INDEX') ? ee()->functions->extract_path($key).'/'.$entry_data->{$prefix.'url_title'} : $entry_data->{$prefix.'url_title'};
			...
			{
				$path = (ee()->functions->extract_path($key) != '' AND ee()->functions->extract_path($key) != 'SITE_INDEX') ? ee()->functions->extract_path($key).'/'.$entry_data->{$prefix.'entry_id'} : $entry_data->{$prefix.'entry_id'};
  			...
  			foreach($entry_data as $key => $value)
  			{
				...
				if (strncmp($prefix.$key, $prefix.'url_title_path', 14) == 0)
				{
  				var_dump($key);exit();
					...
					$tagdata = ee()->TMPL->swap_var_single($key, ee()->functions->create_url($path), $tagdata);
				}
			}
		}
		*/
  			...
  			foreach($channel_fields as $key => $value)
  			{
				...
				if (preg_match_all("/".LD.$prefix.$key."\s+format=[\"'](.*?)[\"']".RD."/s", $tagdata, $matches))
				{
					...
					if(isset($entry_data->{$prefix.$key}))
					{
						...
						{
							$temp_val = ee()->localize->simpl_offset($temp_val, $entry_data->{$prefix.'field_dt_'.$field_id});
							$localize = FALSE;
						}

						if(method_exists(ee()->localize, 'format_date'))
						{
							$val = ee()->localize->format_date($matches[1][0], $temp_val, $localize);
						}
						else
						{
							$val = ee()->localize->convert_timestamp($matches[1][0], $temp_val, $localize);
						}

						$tagdata = ee()->TMPL->swap_var_single($dkey, $val, $tagdata);
					}
				}
			}
		}
	...
	public function parse_single_vars($vars, $entry_data = array(), $channels = array(), $channel_fields = array(), $tagdata = FALSE, $prefix = '', $index = FALSE)
	{
		...
		$prefix_entry = (object) ee()->channel_data->utility->add_prefix($prefix, $entry_data, '');
			...
			if(isset($channel_fields[$field_name]))
			{
				...
				if(isset($entry_data->$field_name) || isset($entry_data->{'field_id_'.$field_id}))
				{
					...
					if(ee()->api_channel_fields->setup_handler($field_id))
					{
						...
						$entry = ee()->api_channel_fields->apply($call_method, array($data, $params, FALSE));

						$tagdata = ee()->TMPL->swap_var_single($single_var, $entry, $tagdata);

						$tagdata = ee()->TMPL->parse_variables_row($tagdata, array($prefix.$field_name => $data));
			...
			foreach($pair_vars as $field_name => $pair_var)
			{
				...
				if((isset($channel_fields[$field_name]) || isset($channel_fields['field_id_'.$field_id])) && (isset($channel_fields[$field_name]->field_type) || isset($channel_fields['field_id_'.$field_id]->field_type)))
				{
					...
					if(ee()->api_channel_fields->setup_handler($field_id))
					{
						...
						ee()->api_channel_fields->apply('_init', array(array('row' => $row)));
						$entry = ee()->api_channel_fields->apply($call_method, array($data, $pair_var[1], $pair_var[0]));
	...
	public function parse_entry($entry_data, $channels = array(), $channel_fields = array(), $tagdata = FALSE, $count = 1)
	{
		...
		ee()->TMPL->template = $this->parse_fieldtypes($channels, $channel_fields);
