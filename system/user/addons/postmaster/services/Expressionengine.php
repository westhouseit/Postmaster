...
class ExpressionEngine_postmaster_service extends Base_service {
	...
	public function send($parsed_object, $parcel, $debug = FALSE)
	{
		...
		ee()->email->from($parsed_object->from_email, $parsed_object->from_name);
		ee()->email->to($parsed_object->to_email, $parsed_object->to_name);
		if(!empty($parsed_object->cc))
		{
			ee()->email->cc($parsed_object->cc);
		}
		ee()->email->subject($parsed_object->subject);
		ee()->email->message($parsed_object->message);

		//[rint_r(ee()->email);
		//die();

		$response = ee()->email->send();
