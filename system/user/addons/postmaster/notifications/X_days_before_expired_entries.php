...
class X_days_before_expired_entries_postmaster_notification extends Base_notification {
	...
	public function send()
	{
		...
		{
			$channel = ee()->channel_data->get_channel_by_name($channel_name);
		...
		$threshold = strtotime($settings->threshold, ee()->localize->now);

		$diff = ee()->localize->now - $threshold;

		$where = array('expiration_date >'  => ee()->localize->now);

		if($diff >= 0)
		{
			$where['expiration_date <='] = ee()->localize->now + $diff;
		}
		else
		{
			$where['expiration_date <='] = ee()->localize->now - $diff;
		...
		$entries = ee()->channel_data->get_entries(array(
		...
		{	
			$this->notification = ee()->postmaster_lib->append($this->notification, 'entry', $entry);
			
			$parse_vars = array();

			$member 	= ee()->channel_data->get_member($entry->author_id);
	...
	private function _update_entry($id, $data = array())
	{
		...
		ee()->db->update('postmaster_expired_entries_emails', $data);
	}
	...
	private function _insert_entry($id, $data = array())
	{
		...
		ee()->db->insert('postmaster_expired_entries_emails', $data);
	}
	...
	private function _existing_entry($id)
	{
		...
		$data = ee()->db->get('postmaster_expired_entries_emails');
