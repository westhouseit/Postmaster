...
class Store_report_products_sold_postmaster_notification extends Base_notification {
	...
	private function _exists($entry_id)
	{
		...
		return ee()->db->get('postmaster_store_report_products_sold')->num_rows() > 0? TRUE : FALSE;
	}

	private function _insert($data)
	{
		ee()->db->insert('postmaster_store_report_products_sold', $data);
	}

	private function _update($entry_id, $data)
	{
		ee()->db->where('entry_id', $entry_id);
		ee()->db->update('postmaster_store_report_products_sold', $data);
	}

	private function _get_entry($entry_id)
	{
		ee()->db->where('entry_id', $entry_id);
		ee()->db->where('date >=', date('Y-m-d 00:00:00', time()));
		
		return ee()->db->get('postmaster_store_report_products_sold');
	}
