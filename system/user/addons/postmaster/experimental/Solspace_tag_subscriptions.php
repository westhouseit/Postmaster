...
class Solspace_tag_subscriptions_postmaster_hook extends Base_hook {
	...
	public function trigger($entry_id, $meta, $data)
	{
		var_dump($data);
					exit();
		if(ee()->session->cache('postmaster', 'entry_trigger')=='new')
		{
			//get tags
			$query = ee()->db->select('tag_entries.tag_id, tag_name')
					->from('tag_entries')
			...
			$q = ee()->db->select('channel_url, comment_url')
					->from('channels')
			...
			{
				$query = ee()->db->select('tag_subscriptions.member_id, screen_name, email')
							->distinct()
				...
				$data['tag_id'] = $tag_id;
				$data['tag'] = $tag_name;
				$data['entry_id_path'] = ee()->functions->create_page_url($basepath, $entry_id);
				$data['url_title_path'] = $data['path'] = ee()->functions->create_page_url($basepath, $data['url_title']);
				foreach ($query->result_array() as $row)
				{
