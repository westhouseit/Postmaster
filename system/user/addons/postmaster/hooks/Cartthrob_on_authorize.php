...
class Cartthrob_on_authorize_postmaster_hook extends Base_hook {
	...
	public function trigger()
	{
		...
		{
			if (ee()->extensions->active_hook('postmaster_cartthrob_processing') === TRUE)
			{
				ee()->extensions->call('postmaster_cartthrob_processing', $this->order);
			}
		}
		...
		foreach($responses as $obj)
		{
			...
			{	
				if($this->end_script($responses))
				{
					...
					ee()->form_builder->set_return($this->cart->order('authorized_redirect'));
