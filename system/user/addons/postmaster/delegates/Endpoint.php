...
class Endpoint_postmaster_delegate extends Postmaster_base_delegate {
	...
	public function email()
	{
	    ...
	    //
	        if (ee()->extensions->active_hook('postmaster_endpoint_send'))
	        {
	            ee()->extensions->call('postmaster_endpoint_send', $data);
	        }
	    //
	    // -------------------------------------------
	...
	private function _get_vars()
	{
		...
		$tmpl_vars = ee()->functions->assign_variables(ee()->TMPL->tagdata);
	...
	{
		preg_match("/(".LD.$var.RD.")(.*)(".LD."\/".$var.RD.")/us", ee()->TMPL->tagdata, $matches);
