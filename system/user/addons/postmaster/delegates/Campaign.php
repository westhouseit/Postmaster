...
class Campaign_postmaster_delegate extends Postmaster_base_delegate {
	...
	private function post($name, $decode = FALSE, $default = FALSE, $xss = TRUE)
	{
		...
		if($decode)
		{
			$value = ee()->base_form->decode($value);
		}
	...
	public function is_subscribed()
	{
		...
		if(ee()->TMPL->tagdata)
		{
			...
			{
				if(preg_match('/\\'.LD.'if '.$no_results_prefix.'no_results\\'.RD.'.*\\'.LD.'\\/if\\'.RD.'/us', ee()->TMPL->tagdata, $matches))
				{
	...
	private function action($subscribe)
	{
		...
		$data          = array(
			'return'     => $this->param('return', ee()->config->site_url()),
		...
		foreach(ee()->TMPL->tagparams as $index => $value)
		{
			if(!isset($data[$index]))
			{
		...
		{
			ee()->functions->redirect($this->param('success_return'));
		}
		
		if(!$response->success && $this->param('failed_return'))
		{
			ee()->functions->redirect($this->param('failed_return'));
		}
	...
	private function form($subscribe, $prefix)
	{
		...
		if($this->validate($this->service, 'subscribe'))
		{
			...
			ee()->base_form->clear();
			ee()->base_form->tagdata = ee()->TMPL->tagdata;
	
			ee()->base_form->set_rule('email', 'required|email');
			
			if((bool) $this->post($prefix.'form'))
			{				
				if(count(ee()->base_form->field_errors) == 0)
				{
					$service = ee()->input->post($prefix.'service');
					$service = ee()->base_form->decode($service);
					$service = $this->lib->load_service($service);
					...
					if(!$response->success)
					{
						...
						{
							ee()->base_form->set_error($error['error']);
						}
					}
					else
					{
					...
					//unset($_POST[ee()->base_form->validation_field]);	
				}				
			}
	...
	private function member_form($subscribe, $prefix)
	{
		...
		if($this->validate($this->service, 'update_member'))
		{
			...
			ee()->TMPL->tagdata = $this->parse(array($vars));
